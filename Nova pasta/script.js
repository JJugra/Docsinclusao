...
async function enviarParaSupabase() {
  ...
  try {
    ...
    showToast("Dados salvos com sucesso!", "success", 5000);
...
async function carregarSemanarios() {
  try {
    // 1. Pega os dados do professor que acabou de logar (armazenados no localStorage)
    const cache = localStorage.getItem("usuarioAtivo");
    if (!cache) {
      console.warn("Nenhum usuário logado encontrado no cache.");
      return;
    }

    const usuarioLogado = JSON.parse(cache);
    const nomeProfessor = usuarioLogado.nome; // O nome vindo da tabela 'semanariousers'

    console.log("Iniciando busca para:", nomeProfessor);

    // 2. Procura na tabela 'semanarios' onde o nome seja IGUAL ao logado
    const { data, error } = await supabaseClient
      .from("semanarios")
      .select("*")
      .eq("professor_adj", nomeProfessor); // Filtro puro e direto

    // 3. Se o Supabase responder com erro (ex: coluna errada), ele cai aqui
    if (error) throw error;

    // 4. Se encontrar dados, ele segue para a função que desenha a lista na tela
    if (data && data.length > 0) {
      console.log(`${data.length} registros encontrados para este professor.`);
      exibirLista(data);
    } else {
      console.log(
        "Nenhum dado encontrado para este nome na tabela semanarios.",
      );
      exibirLista([]); // Limpa a lista se não houver dados
    }
  } catch (error) {
    // Aqui ele vai te dizer se o erro é o nome da coluna 'professor_adj' ou outro problema
    console.error("Erro técnico ao carregar:", error.message);
  }
}
